...
async def update_medicine(medicine_id: int, medicine_update: dict):
    """更新藥物資訊"""
    for i, medicine in enumerate(medicines_db):
        if medicine["id"] == medicine_id:
            # 更新指定欄位
            for key, value in medicine_update.items():
                if key in medicine and key != "id" and key != "create_time":
                    medicine[key] = value
...
async def delete_medicine(medicine_id: int):
    ...
    return {"success": True, "message": "藥物刪除成功"}
