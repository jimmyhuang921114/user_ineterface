...
(static_dir / "html").mkdir(exist_ok=True)
...
@app.get("/")
async def root():
    return {"message": "醫院藥物管理系統API", "status": "running", "version": "1.0"}

@app.get("/api/test")
async def test_api():
    return {"status": "success", "message": "API連接正常"}
...
async def create_medicine(medicine: dict):
    medicine["id"] = len(medicines_db) + 1
...
async def delete_medicine(medicine_id: int):
    global medicines_db
    medicines_db = [m for m in medicines_db if m.get("id") != medicine_id]
    return {"message": "Medicine deleted successfully"}

# 服務HTML文件
@app.get("/Medicine.html")
async def serve_medicine_html():
    html_file = static_dir / "html" / "Medicine.html"
    if html_file.exists():
        return FileResponse(html_file)
    else:
        return JSONResponse(
            content={"error": "Medicine.html not found"}, 
            status_code=404
        )
