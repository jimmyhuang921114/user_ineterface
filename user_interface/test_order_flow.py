...
class OrderFlowTester:
    ...
    def check_server_status(self) -> bool:
        """檢查服務器狀態"""
        try:
            response = self.session.get(f"{self.base_url}/api/system/status", timeout=)
    ...
    def create_test_medicine(self, name: str, description: str = "測試用藥物", stock: int = ) -> bool:
        """創建測試藥物"""
        try:
            data = {
                "name": name,
    ...
    def run_basic_test(self):
        ...
        if not prescription_id:
         ...
         . 監控狀態變化
        final_status = self.monitor_prescription_status(prescription_id, timeout=)
    ...
    def run_batch_test(self, count: int = ):
        ...
        for prescription_id in prescription_ids:
            ...
            status = self.monitor_prescription_status(prescription_id, timeout=)
            if status == 'completed':
                completed_count += 
