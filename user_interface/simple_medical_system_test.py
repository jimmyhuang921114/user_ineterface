...
"""
簡化醫療系統測試 (不依賴ROS2)
Simple Medical System Test (No ROS2 dependency)

測試流程：
1. 寫入病例
2. 調取病例和藥物資料 (模擬ROS2調用)
3. 回報完成狀態
"""
...
def simulate_medical_data_query(query_type, query_id, patient_id="", additional_params=""):
    ...
    try:
        if query_type == "medical_record":
            if patient_id:
                url = f"{API_BASE}/api/medical_record/patient/{patient_id}"
            else:
                url = f"{API_BASE}/api/medical_record/{query_id}"
        elif query_type == "medicine":
            url = f"{API_BASE}/api/medicine/integrated/{query_id}"
        elif query_type == "prescription":
            url = f"{API_BASE}/api/prescription/{query_id}"
        else:
        ...
        print(f"✓ 查詢成功!")
        print(f"數據預覽: {response.data[:200]}...")
...
def main():
    ...
    print("\nAPI端點測試：")
    print("- /api/medical_record/ (POST, GET)")
    print("- /api/medical_record/{id} (GET)")
    print("- /api/medical_record/patient/{patient_id} (GET)")
    print("- /api/medicine/integrated/{name} (GET)")
    print("- /api/medical_record/stats/summary (GET)")
