        ...
        async def get_next_order():
            ...
            try:
                if not self.order_pusher:
                    raise HTTPException(status_code=, detail="Order pusher not ready")
                ...
                if not pending:
                    return JSONResponse(status_code=, content={})   No content
                
                 取最舊的一個
                oldest = min(pending, key=lambda x: x.get('id', ))
